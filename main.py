...
def display_component():
    ...
    for i in range(len(most_recent_challenges)):
        ...
        challenger, c_rank = challenger.rsplit(" ", 1)
        opponent, o_rank = opponent.rsplit(" ", 1)
...
def new_challenge_component():
    ...
    elif date_obj < (datetime.today() - timedelta(days=1)):
...
def queries_component():
    ...
    if option == 1:
        ...
        print(matches)
        done_records = [
            f"{record[0].rsplit(' ', 1)[0]} v {record[1].rsplit(' ', 1)[0]} ({record[2]}) - {record[3]}"
            for record in matches
            if len(record) == 4
        ]
        yet_to_play_records = [
            f"{record[0].rsplit(' ', 1)[0]} v {record[1].rsplit(' ', 1)[0]} ({record[2]})"
            for record in matches
            if len(record) == 3
        ]
        ...
        yet_to_play_records = [
            f"{record[0]} v {record[1]} ({record[2]})"
            for record in matches
            if len(record) == 3
        ]
        ...
        turtle.textinput(
            "Most Active Player",
            f"The most active player is {', '.join(players)} ({count} matches)",
        )
        ...
        turtle.textinput(
            "Least Active Player",
            f"The least active player is {', '.join(players)} ({count} matches)",
        )
        ...
        turtle.textinput(
            ...
            f"The player with most wins is {', '.join(players)} ({count} wins)",
        )

    elif option == 9:
        players, count = actions.get_player_with_least_wins()
        turtle.textinput(
            "Player With Least Wins",
            f"The player with least wins is {', '.join(players)} ({count} wins)",
        )


def check_if_menu_buttons_clicked(x, y):
    # calls the corresponding buttons when the menu buttons are clicked
...
def refresh_screen():
    # clear the screen and redraw the components with the updated data
    screen.clear()
    screen.tracer(False)
    display_component()
